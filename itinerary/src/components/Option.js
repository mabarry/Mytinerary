...
function Option({ onClick, optionInfo }) {
  const [name, setName] = useState(optionInfo.name)
  const [img, setImg] = useState(optionInfo.image_url)
  const [rating, setRating] = useState(optionInfo.rating)
  ...
  return (
    <div className="option-box" onClick={handleOptionClick}>
      <img src={img}  style={{width: "80%", borderRadius: "16px"}}/>
      <h3>{name}</h3>
